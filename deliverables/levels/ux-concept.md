...
английский (слова и каламбуры); подписи интерфейса — русские (лягут в прототип).
...
│                          │
│   ┌──────────────────┐   │
│   │     ИГРАТЬ  ▶     │   │  ← ЗОНА БОЛЬШОГО ПАЛЬЦА
│   └──────────────────┘   │     (нижняя треть)
│   [🔊]   [💰 250]   [⚙]   │  ← валюта видна с первого экрана
╰──────────────────────────╯
```
- **Почему кнопка ИГРАТЬ в нижней трети:** зона уверенного дотягивания большого пальца в portrait
...
│                          │
│   [🔀 Перемешать]  [💡]   │  Shuffle бесплатно + лестница подсказок
╰──────────────────────────╯
```
...
- **Почему конфетти + бегущие монеты:** «Peggle-момент» завершения (research §6) — эмоциональная
  точка, ради которой играют. Кнопка ДАЛЕЕ ведёт назад на тропу к следующему узлу.
...
| Подсказка (RU-подпись) | Когда | Цена (модель полной версии) | Что делает |
|---|---|---|---|
| **🔀 Перемешать** | всегда, обе фазы | **0 (бесплатно)** | перемешивает пул |
| **💡 Открыть букву** | обе фазы | дёшево (~25) | ставит одну случайную верную букву в слот |
| **🎯 Буква на позиции** | обе фазы | дороже (~2×) | игрок тапает пустой слот → туда встаёт верная буква |
| **❓ Объяснить шутку** | только фаза ответа | средне | показывает `surprise.explanation` — нарративный хинт |
| **✨ Решить слово** | обе фазы | дорого / rewarded ad | достраивает всё текущее слово |
...
| Похвала | эскалация эпитетов: **ОТЛИЧНО! → БЛЕСТЯЩЕ! → ВЕЛИКОЛЕПНО!** | research §6 |
